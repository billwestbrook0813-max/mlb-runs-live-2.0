  ...
  try {
    ...
    for (const ev of events) {
      ...
      for (const bk of ev.bookmakers ?? []) {
        ...
        allPts.push(pt); if (started) { const lastUpd = m.last_update || bk.last_update || null; if (recent(lastUpd)) livePts.push(pt); }
      }
      const used = (started && livePts.length) ? livePts : allPts;
      if (!used.length) continue;
      games.push({ id: ev.id, status: started ? "live-or-started" : "pre", commence_time: ev.commence_time, home_team: ev.home_team, away_team: ev.away_team, bookmakers_count: used.length, consensus_total: Number(mean(used).toFixed(2)), consensus_std: Number(std(used).toFixed(2)) });
    }
    ...
    const payload = { datePT: today, projectedRuns: projectedMean, projectedStd, bandLow: Number((projectedMean - projectedStd).toFixed(1)), bandHigh: Number((projectedMean + projectedStd).toFixed(1)), gameCountUsed: games.length, games, source: "The Odds API (live-aware)", lastUpdateUtc: new Date().toISOString() };
